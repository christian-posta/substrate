...
Each worker's Redis record (an `ateapipb.Worker`) carries, among other fields:
...
inserts an idle worker record into Redis (stamping in the pool's
`sandbox_class`, `node_name`, and `labels`).
...
`FinalizeSuspendedStep` clears the worker's `assignment` (and the actor's
`worker_pool_name`). The worker is immediately eligible for the next actor.
