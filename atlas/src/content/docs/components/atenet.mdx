...
polling for ready templates (a periodic poll, not an informer-driven
watch).
...
2. Asks ateapi to resume that actor, referencing it by `ObjectRef{atespace, name}`.
   Concurrent requests for the same actor collapse into a single resume.
3. Receives the `Actor` back and reads `ateom_pod_ip`.
4. Returns a `HeaderMutation` telling the proxy to overwrite `:authority`
   with `<pod_ip>:80`. (The `:80` is currently hardcoded; supporting other
   actor ports is a known follow-up.)
