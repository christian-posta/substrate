...
container reports 200** (a readiness wait).

### Checkpoint - snapshot the VM
...
this actor's, then **relaunches CH and issues a `vm.restore` over the REST
api-socket** with `memory_restore_mode: OnDemand` (userfaultfd lazy paging) and
the fresh tap FDs passed as SCM_RIGHTS, then resumes. (CH is launched only with
`--api-socket`; the restore is a REST call, not a CLI flag.) Guest RAM -
including in-memory state, the tmpfs rootfs upper,
...
host-bind-mounted into the pod, addressed by pod UID. (The base path is shared
