...
flowchart LR
  ...
  SYNC[Worker-pool syncer] -- write --> R
  WF[Workflow engine] -- read/write/lock --> R
  R -- worker updates --> WC[In-memory worker cache]
...
worker cache that streams changes over Redis pub/sub and
...
Both back-ends implement the same internal object interface. The choice is
environmental - GCS in GCP, S3 in AWS - selected at
