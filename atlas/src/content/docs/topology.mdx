...
flowchart LR
  ...
  ROUTER["atenet router<br/>Envoy :8080/:8443<br/>ExtProc :50051"]
  ATEAPI["ateapi"]
...
| **atenet** | Deployments (one binary, two subcommands) | `atenet router` runs Envoy + ExtProc and does the per-request dance to find the right worker. `atenet dns` programs CoreDNS so `<actorId>.actors.resources.substrate.ate.dev` resolves. Each subcommand is typically deployed as its own Deployment. |
...
| ateapi | 443 | gRPC (TLS) |
| atelet | 8085 | gRPC |
| atenet router (Envoy) | 8080 / 8443 | HTTP / HTTPS |
| atenet ExtProc | 50051 | gRPC (Envoy ext_proc) |
| atenet xDS | 18000 | gRPC (Envoy control) |
| ateom-gvisor | Unix socket | gRPC — `/var/lib/ateom-gvisor/ateoms/{podUID}/ateom.sock` |

## How traffic *actually* gets to an actor

The short version: there is **no per-pod sidecar**. atenet's centralized Envoy
is the single dataplane proxy, and ExtProc consults ateapi on each request to
discover (and if needed, resume) the actor's worker.
