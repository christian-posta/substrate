...
- Any subsequent `CreateActor + ResumeActor` for an actor of this template
  ...
  own newer snapshot. `CallAteletRestoreStep` picks the strategy in this
  ...
  with `boot=true`), else a cold boot from the template spec. See
...
- If **every** container in the template declares a `readyz` HTTP probe,
  `goldenSnapshotWarmupFor` returns **0**. `ResumeActor` already blocks until
  each container's `readyz` reports 200 (both runtimes call `readyz.WaitAll`
  before `Run`/`Restore` returns), so the workload is already initialized by
  the time the controller gets `RUNNING` back - no extra wait is needed.
- Otherwise (any container lacks a probe, or the template declares no
  containers), it falls back to the default **`goldenSnapshotWarmup`
  = 20 s** as a coarse "give the workload time to initialize" budget.
