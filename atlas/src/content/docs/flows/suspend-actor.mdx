...
sequenceDiagram
  ...
  rect rgb(245,235,235)
    ...
    O-->>L: SnapshotFiles[]
    L->>G: zstd-compress + upload in parallel (errgroup)<br/>+ manifest.json
    G-->>L: ok
    L-->>A: Checkpoint ok

    A->>R: FinalizeSuspended<br/>(clear worker Assignment,<br/>latest_snapshot_info=External{URI}, status=SUSPENDED)
  end

  A-->>C: Actor{ status=SUSPENDED, latest_snapshot_info=External }
```

**Pause** follows the same shape, with a different tail: `PauseActor` →
`MarkPausing` (status `PAUSING`) → `Checkpoint(LOCAL, snapshotPrefix,
scope=onPause)` → atelet moves the checkpoint into the node's local-checkpoint
directory instead of uploading → `FinalizePaused` sets status `PAUSED` and
writes a `LocalSnapshotInfo` recording the node VM that holds the snapshot.

## What happens in each step

### MarkSuspending / MarkPausing - reserve the snapshot location
...
### CallAteletSuspend / CallAteletPause - drive the checkpoint

ateapi resolves the **atelet DaemonSet pod running on the same node** as the
actor's worker pod - dialing via `DialForWorker(actor.AteomPodNamespace,
actor.AteomPodName)`, which looks up the atelet on that worker's node and dials
its pod IP on `:8085`. atelet is per-node, not per-pod: one atelet serves every
worker pod scheduled to its node.

If the actor is in `SUSPENDING`/`PAUSING` but has no assigned worker
(`ateom_pod_namespace`/`ateom_pod_name` empty), the step crashes the actor
(`crashActor`) and fails - there's nothing to checkpoint.
...
1. Calls `ateom-gvisor.CheckpointWorkload` over a Unix socket in the shared
...
2. ateom-gvisor execs `runsc` to checkpoint into
   `/var/lib/ateom-gvisor/actors/<atespace>:<name>/checkpoint-state/` on the
...
3. **Suspend** zstd-compresses each reported file and uploads it to GCS/S3 **in
   parallel** using an `errgroup`, then writes a `manifest.json` alongside them.
   **Pause** instead moves the checkpoint into the node's
   `.../local-checkpoint/` directory - no upload.

<code class="fileref">cmd/ateapi/internal/controlapi/workflow_suspend.go</code> ·
<code class="fileref">cmd/ateapi/internal/controlapi/workflow_pause.go</code> ·
<code class="fileref">cmd/ateapi/internal/controlapi/dialer.go</code> ·
<code class="fileref">cmd/atelet/main.go</code> ·
<code class="fileref">cmd/ateom-gvisor/main.go</code>

### FinalizeSuspended / FinalizePaused - release the worker
...
Once `ateom-gvisor.CheckpointWorkload` runs, the sequence inside the worker pod
is:
...
3. The **actor veth pair is torn down** (`cleanupActorNetwork`): ateom deletes
   the host-side veth `ateom0` (which also removes its peer) and any interior
   veth, undoing the link-local `169.254.17.0/30` wiring and the `ateom_actor`
   nftables rules. (This is the current model - the actor no longer moves a real
   `eth0` between netns; the pod keeps its own `eth0` throughout.)
...
Crucially, this **does not** propagate to the client as an error. The router's
`ActorResumer` wraps the call in a `singleflight.Group` (keyed
`<atespace>/<actor_name>`) plus an explicit retry loop: on `Aborted` it backs
off and retries with an exponential backoff (7 steps, starting at 200ms, factor
1.5, jitter 0.2, capped by a 15s background context). All concurrent requests
for the same actor share the single in-flight call via `singleflight.DoChan`, so
the freeze → resume transition causes one resume to happen and every waiting
request piggybacks on it.
...
stuck checkpoint. If that happens, the gRPC `Aborted` falls through
`mapResumeError`'s default branch and the client gets HTTP 500.
...
| Still in ExtProc / not yet routed | `ResumeActor` returns `Aborted`, router's `singleflight` + backoff retries until the freeze completes, then routes to a worker. Added latency, no error. |
...
- **Lock conflict**: a concurrent operation on the same actor returns gRPC
  ...
  back off and retry.
- **External upload fails (suspend)**: atelet returns `codes.DataLoss` carrying
  the `actorCrashed` directive; ateapi's `maybeCrashActor` moves the actor to
  **`CRASHED`** (external snapshot files aren't cached on failure, so the
  workflow can't safely retry - tracked as
...
- **No worker at checkpoint time**: if the actor is `SUSPENDING`/`PAUSING` but
  has no assigned worker, the workflow crashes the actor (`crashActor`) - see
...
- **Worker pod dies mid-checkpoint**: the syncer's `releaseActorOnDeadWorker`
  ...
  `in_progress_snapshot` and the `ateom_pod_*`/`worker_pool_name` pointers, and
