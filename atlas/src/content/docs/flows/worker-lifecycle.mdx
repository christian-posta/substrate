...
```mermaid
flowchart LR
  K[K8s pod informer] -- Add/Update<br/>(PodIP assigned) --> C[Create worker record<br/>assignment=nil<br/>+ cache node_name,<br/>sandbox_class, labels]
  K -- pod gone<br/>(delete/soft-delete) --> R[dead-worker recovery<br/>if assignment set:<br/>actor → SUSPENDED]
  R --> D[Delete worker record]
  C -.-> RED[(Redis)]
  D -.-> RED
  R -.-> RED
```
...
flowchart LR
  ...
  DEP --> PN[...]
  P1 -- has PodIP --> SYNC[worker-pool syncer]
  P2 -- has PodIP --> SYNC
  PN -- has PodIP --> SYNC
  SYNC -- create worker record --> RED[(Redis: Idle worker)]
```
