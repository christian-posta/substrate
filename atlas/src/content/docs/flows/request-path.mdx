---
title: Request path
description: How an inbound HTTP request gets from a client to the right actor on the right worker. No per-pod sidecar — a single Envoy fleet + ExtProc + ateapi lookup.
---
...
discovers the worker IP for a given actor, what Envoy is configured to do, and
what happens when the actor is already running (the warm path).
...
There is **one Envoy Deployment** for the whole cluster (atenet router).
...
flowchart LR
  ...
  RED --> API
  API --> EXT
  EXT -->|HeaderMutation:<br/>:authority=ip:80| ENV[Envoy]
  ENV --> WK[Worker]
```
...
Envoy is bootstrapped pointing at the atenet router's **xDS server on
...
| Cluster `dynamic_forward_proxy_cluster` | Forwards to whatever :authority Envoy now holds |
...
`:authority` so Envoy's `dynamic_forward_proxy_cluster` hits it.
