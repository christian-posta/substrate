...
```mermaid
flowchart LR
  REQ[Request] --> EXT[ExtProc<br/>request headers]
...
would otherwise produce 50 `ResumeActor` calls. ExtProc dedupes concurrent
calls for the same actor by keying on `<atespace>/<actor_name>` - the first call
goes through, the rest piggyback on its result.
...
also runs the `otelgrpc` stats handler. On the control-plane side, `ateapi`
now tags its own spans with the actor's `ate.*` telemetry identity (atespace,
name), so a resume can be followed end to end across the router and ateapi.
...
re-reads the ready templates and rebuilds the snapshot - it's a
...
The router translates the gRPC status from ateapi into an HTTP status
(the original error is preserved for logging):
...
the router retries it internally with backoff.
