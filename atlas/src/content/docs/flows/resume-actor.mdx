...
sequenceDiagram
  ...
  Note over X: handleRequestHeaders()<br/>parses host → (atespace, actor_name)

  X->>A: ResumeActor(ObjectRef{atespace, name})<br/>(via singleflight dedupe, key atespace/name)
  ...
  rect rgb(240,240,255)
    ...
    A->>R: AssignWorker: pick random idle eligible worker<br/>(local snapshot? restrict to nodes that hold it)
...
(or `Host`) header into an **`(atespace, actor_name)`** pair via
`resources.ParseActorDNSName` (port stripped); an unparseable host is answered
with a 404.

<code class="fileref">cmd/atenet/internal/router/extproc.go</code>
(`handleRequestHeaders` lives in `extproc.go`; `extproc_in.go` holds the
`requestMetadata` / `parseActorRef` helpers.)

### 6. ResumeActor (with singleflight)

ExtProc calls `ateapi.ResumeActor` with an `ObjectRef{atespace, name}`. To avoid
stampedes when 50 concurrent requests hit a cold actor, ExtProc wraps the call
in a `singleflight.Group` keyed by `<atespace>/<actor_name>` so only the first
call goes through; the rest piggyback on its result. The in-flight resume runs
under a detached background timeout so a caller disconnecting doesn't abort the
resume for the others, and `Aborted` (a concurrent resume) is retried with
backoff. ExtProc also opens an OTel span here.
...
2. **AssignWorker** - random shuffle over idle **eligible** workers (the
...
else                                          → atelet.Run (cold boot from spec)
```
...
atelet's `Restore` handler first fetches the small `manifest.json` (which pins
...
`/var/lib/ateom-gvisor/actors/<atespace>:<name>/restore-state/`. For a
...
- **ateom-gvisor and the actor live at different layers.** ateom-gvisor is
  ...
  *runsc-managed gVisor sandbox* that ateom-gvisor spawns - a sibling
  ...
  into existence.
- **The snapshot images are downloaded in parallel** via an `errgroup` and
  streamed through `zstd` on the way to disk. atelet doesn't assume a fixed
  file list - the manifest tells it exactly which files the runtime wrote.
- **The download and the bundle prep overlap.** atelet runs the checkpoint
  ...
  unpacking the OCI image, since only the final `RestoreWorkload` needs both.
...
- **`RestoreWorkload` still waits on readyz.** After the restore exec returns,
  ateom-gvisor blocks until every readyz-enabled container reports HTTP 200
  (`internal/readyz`, probing the actor's veth IP) before it returns `ok` to
  atelet. So ateapi finalizes the actor to `RUNNING` only once the workload is
  actually serving.
...
returns `codes.FailedPrecondition` (`"no free workers available"`), which
ExtProc maps to HTTP **503 ServiceUnavailable**.
