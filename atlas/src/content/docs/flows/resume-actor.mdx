...
touches DNS, Envoy, ExtProc, ateapi's workflow engine, the Redis store,
...
sequenceDiagram
  ...
  participant E as Envoy<br/>(atenet router)
...
Envoy accepts the request on `:8080` and applies a single `ext_proc` filter
...
ExtProc emits a header-mutation response telling Envoy to overwrite
`:authority` with `<pod_ip>:80`. Envoy then forwards the request to the
worker.
...
- **No per-pod sidecar.** A single Envoy fleet handles all routing.
