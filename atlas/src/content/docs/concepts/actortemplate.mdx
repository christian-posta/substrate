...
`ActorTemplate.spec` is validated to reject any edit after creation. Changing
an image would invalidate the golden snapshot, so a new "version" means a
**new template**, not an edit.
:::
...
- `command[]` / `args[]` - override the image entrypoint, mirroring Kubernetes
  semantics: `command` overrides the image `ENTRYPOINT` and `args` overrides its
  `CMD`. If `command` is set, both the image `ENTRYPOINT` and `CMD` are ignored
  and the process argv is `command + args`. Unlike Kubernetes, `$(VAR)`
...
reserved `ate-golden` atespace (see the [Atespace](/concepts/atespace/) page):
