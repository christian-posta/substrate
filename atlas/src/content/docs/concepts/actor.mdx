...
The `Actor.Status` enum defines eight values
...
(suffix `ActorDNSSuffix` and helper `ActorDNSName(atespace, actorName)` are
defined in `internal/resources/actor.go`).
...
- `Actor` message, `Status` enum, `SnapshotInfo`: <code class="fileref">pkg/proto/ateapipb/ateapi.proto</code>
- DNS name + reserved golden atespace: <code class="fileref">internal/resources/actor.go</code>
- Redis key layout (`actor:<atespace>:<name>`): <code class="fileref">cmd/ateapi/internal/store/ateredis/ateredis.go</code>
:::
