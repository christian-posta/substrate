...
`SnapshotInfo` (`pkg/proto/ateapipb/ateapi.proto`) is a `oneof data` with two
...
- **gVisor** (`ateom-gvisor`, `runsc checkpoint`/`restore`): a `checkpoint.img`
  ...
  hardcoded list (`cmd/ateom-gvisor/main.go`), so resume can stream RAM pages
  in lazily instead of reading the whole image up front.
- **micro-VM** (`ateom-microvm`, cloud-hypervisor): a `config.json`, a
  ...
  captured by the memory snapshot (`cmd/ateom-microvm/checkpoint.go`).
...
- `SnapshotInfo` / `ExternalSnapshotInfo` / `LocalSnapshotInfo`: <code class="fileref">pkg/proto/ateapipb/ateapi.proto</code>
- `SnapshotScope`, `SnapshotsConfig` (`onPause`/`onCommit`): <code class="fileref">pkg/api/v1alpha1/actortemplate_types.go</code>
- gVisor checkpoint files: <code class="fileref">cmd/ateom-gvisor/main.go</code>; micro-VM: <code class="fileref">cmd/ateom-microvm/checkpoint.go</code>
- upload + sparse-zstd: <code class="fileref">cmd/atelet/main.go</code>, <code class="fileref">cmd/atelet/internal/ategcs/sparsezstd.go</code>
:::
