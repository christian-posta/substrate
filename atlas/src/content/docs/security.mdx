...
- **Store transport:** ValKey (v9.1) is fronted by TLS and reloads its certs
  ...
  authenticate to it with Google Cloud **IAM** (`--redis-use-iam-auth`, a string
  flag defaulting to `"true"`; IAM is used unless it is set to `"false"`), and it
  is the only component that connects to the database. See
...
- **Cryptographic snapshot verification** before restore - today the gVisor
  ...
  verified digest.
- **Deprivileged worker pods** - the worker pods still run `privileged: true` to
  manage sandboxes and namespaces (the `atelet` DaemonSet was recently
  deprivileged, but the workers themselves have not been yet).
